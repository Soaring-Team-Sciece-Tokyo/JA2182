...
このリポジトリは、東京科学大学グライダー部のフライトシミュレータ関連コードを保守・改造するための作業用リポジトリです。
...
普段使う想定はPython実行ではなくEXE配布版（部室のPCのデスクトップにインストール済み）です。  
...
- デスクトップにおいてあるのはショートカットなので、元のフォルダの方を探してください。そこにあるはず。
